...
uint8_t XBOXUSB::Init(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        if(VID != XBOX_VID && VID != MADCATZ_VID && VID != JOYTECH_VID && VID != GAMESTOP_VID && VID != LOGITECH_VID) // Check VID
...
#endif
                goto FailUnknownDevice;
        } else if(PID != XBOX_WIRED_PID && PID != MADCATZ_WIRED_PID && PID != GAMESTOP_WIRED_PID && PID != AFTERGLOW_WIRED_PID && VID != LOGITECH_VID) // Check PID
                goto FailUnknownDevice;
